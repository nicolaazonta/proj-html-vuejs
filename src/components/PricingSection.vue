<script>
export default {
    name: 'PricingSection'
}
</script>

<template>
    <div id="PricingSection">
        <div class="container w-75 padding_special">
            <h6 class="section_subtitle text-center">monthly plans</h6>
            <h2 class="section_title text-center"><span class="marked">Pricing</span> Table</h2>
            <p class="text-center px-5">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Excepturi sapiente et
                facilis amet magnam odit dignissimos voluptatibus voluptates vel. Corporis?</p>
            <div class="row py-3 align-items-center">
                <div class="col-4">
                    <div class="card d-flex flex-column align-items-center p-3">
                        <font-awesome-icon class=" icon" icon="fa-solid fa-user-group" />
                        <h5 class="text-capitalize">standard</h5>
                        <span class="price">$<span class="big_price">490</span>/mo</span>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                        
                        
                    </div>
                </div>
                <div class="col-4">
                    <div class="card d-flex flex-column align-items-center px-3 py-4">
                        <font-awesome-icon class=" icon" icon="fa-solid fa-user-group" />
                        <h5 class="text-capitalize">professional</h5>
                        <span class="price">$<span class="big_price">890</span>/mo</span>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                    </div>
                </div>
                <div class="col-4">
                    <div class="card d-flex flex-column align-items-center p-3">
                        <font-awesome-icon class=" icon" icon="fa-solid fa-user-group" />
                        <h5 class="text-capitalize">ultimate</h5>
                        <span class="price">$<span class="big_price">1.390</span>/mo</span>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                        <div class="spec d-flex justify-content-between">
                            <span class="text-capitalize spec_text">standard accounting</span>                        
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>


<style lang="scss"></style>